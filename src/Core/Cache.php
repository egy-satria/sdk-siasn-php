...
class Cache
{
    ...
    public function isNotExpired($cache_name)
    {
        $cachedData = $this->_loadCache();
        if (!isset($cachedData[$cache_name])) {
            return false;
        }

        return (false === $this->_checkExpired($cachedData[$cache_name]['time'], $cachedData[$cache_name]['expire']));
    }
